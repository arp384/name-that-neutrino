...
class CorsikaLabeler(icetray.I3Module):
    def __init__(self, context):
    ...
    def classify(self, frame):
        ...
        if len(poly_muons) != 0:
            cr_int_t, cr_containment = self._classify_corsika(frame)
        else:
            
            cr_int_t, cr_containment = (None, None)
        #print(cr_int_t)
        #print(cr_containment)
        cr_class = class_mapping.get((cr_int_t, cr_containment), classification.unclassified)
        #print(cr_class)
    ...
    def DAQ(self, frame):
        ...
        cr_class, n_coinc, mcbg_q_from_muons = self.classify(frame)
        frame["coincident_muons" + self._key_postfix] = icetray.I3Int(n_coinc)
        frame["corsika_label" + self._key_postfix] = icetray.I3Int(int(cr_class))
        frame["corsika_classification" + self._key_postfix] = dataclasses.I3String(cr_class.name)
        frame["bg_charge" + self._key_postfix] = dataclasses.I3Double(mcbg_q_from_muons)
